...
graph = get_classes_graph(obj)

def get_classes_leafs(untangle_obj):
	return [str(Class['id']) for Class in untangle_obj.Class if graph[str(Class['id'])]['childs'] == '']

def add_inherited_methods_to_classes(graph):
	def add_inherited_methods_to_class(base_id, child_id, untangle_obj):
		def get_inherited_methods(base_id, child_id, untangle_obj):
			...
			bases_members = [get_method_name_by_id(x) for x in graph[base_id]['members'].split(' ') if x in methods_list]
			childs_members = [get_method_name_by_id(x) for x in graph[child_id]['members'].split(' ') if x in methods_list]
	...
	to_view = get_classes_roots_list(obj)
	while to_view != []:
		...
		to_view += childs
		for child in childs:
			add_inherited_methods_to_class(base_class, child, obj)

#print graph['_130']['childs']
	
add_inherited_methods_to_classes(graph)
for Class in get_classes_id_list(obj):
	print Class + ':' + graph[Class]['members']

def restructuring_classes_graph():
...
restructuring_classes_graph()
for Class in get_classes_id_list(obj):
	print Class + ':' + graph[Class]['members']

def generate_java_file(f_name):
	def write_class_to_file(class_id):
		def get_method_name_by_id(method_id):
			return [str(Method['name']) for Method in obj.Method if Method['id'] == method_id][0]
	...
	method_list = [str(Method['id']) for Method in obj.Method]
...
generate_java_file('test')
'''
for Class in obj.Class:
	print type(Class)
for Class in obj.Class:
	print ('%s - %s', Class['name'], Class['childs'])

for Class in obj.Class:
	print Class['bases']
print obj.Class['id']
print obj.Class['name']
print obj.Class['members']
print obj.GCC_XML.Class.Base['access']

def get_method_by_id(method_id):
	for Method, Constructor in obj:
		if obj.Method['id'] == method_id:
			return obj.Method
		elif obj.Constructor['id'] == method_id:
